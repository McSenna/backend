...
// Registration requires a completed email-OTP step. The OTP arrives by email, so seed the verified
// state directly and hand back the token the client would normally receive.
...
async function verifiedRegistration(key, overrides = {}) {
  const payload = registrationPayload(key, overrides);
  return { ...payload, emailVerificationToken: await verifiedEmailToken(payload.email) };
}
...
async function cleanup() {
  ...
  await EmailVerification.deleteMany({ email: new RegExp(`^${TAG}\\.`, "i") });
...
async function run() {
  ...
  const registered = await request("POST", "/register", {
    body: await verifiedRegistration("approve"),
    headers: NATIVE,
  });
  check(
  ...
  const secondRegistration = await request("POST", "/register", {
    body: await verifiedRegistration("reject", {
